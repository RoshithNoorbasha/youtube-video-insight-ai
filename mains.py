...
def transcribe_video(video_id):
    try:
        ...
        # List all available languages for the video
        ...
        if 'en' in available_languages:
            ...
            # Fallback: Get first available transcript and translate to English
            transcript = transcript_list.find_transcript(available_languages)
            original = transcript.fetch()
            try:
                translated = transcript.translate('en').fetch()
            except Exception:
                translated = original  # If translation fails, use original
        ...
        if "Too Many Requests" in error_message:
            st.error("YouTube is currently rate limiting transcript requests. Please wait a few minutes before trying again.")
        else:
